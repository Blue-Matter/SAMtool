...
rmd_RCM_survey_output <- function(sur, s_name) {
  ans <- c(paste0("### ", s_name[sur], " \n"),
           ...
           paste0("if(!is.null(RCMdata@IAA) && any(RCMdata@IAA[, , ", sur, "] > 0, na.rm = TRUE)) {"),
           paste0("MApred <- do.call(cbind, lapply(report_list, function(x) x$IAApred[, , ", sur, "] %*% age/rowSums(x$N[1:nyears, ] * x$s_vul[1:nyears, , ", sur, "])))"),
           ...
           paste0("if(!is.null(RCMdata@IAL) && any(RCMdata@IAL[, , ", sur, "] > 0, na.rm = TRUE)) {"),
           paste0("MLpred <- do.call(cbind, lapply(report_list, function(x) x$IALpred[, , ", sur, "] %*% length_bin/rowSums(x$N[1:nyears, ] * x$s_vul[1:nyears, , ", sur, "])))"),
...
rmd_RCM_Hist_compare <- function() {
  c("## Updated OM {.tabset}\n",
    ...
    "lines(Year, rowSums(Chist, na.rm = TRUE), col = \"black\")",
    "abline(h = 0, col = \"grey\")",
    "legend(\"topleft\", c(\"RCM\", \"OM\"), col = c(\"black\", \"red\"), pch = c(NA_integer_, 16), lwd = c(1, 1), bty = \"n\")",
    "```\n",
    "",
    ...
    "if(any(Chist > 0, na.rm = TRUE)) {",
    "Catch_difference <- t(Hist_C)/rowSums(Chist, na.rm = TRUE) - 1",
