...
rmd_RCM_Perr <- function(fig.cap = "Historical recruitment deviations among simulations.") {
  c(paste0("```{r, fig.cap = \"", fig.cap, "\"}"),
    "Perr <- OM@cpars$Perr_y[, max_age + seq(1, nyears), drop = FALSE]",
    ...
    "Perr_future <- OM@cpars$Perr_y[1:min(5, nrow(OM@cpars$Perr_y)), max_age + nyears + seq(1, proyears), drop = FALSE]",
    "matplot(Year, t(Perr), type = \"l\", col = \"black\", xlab = \"Year\", ylab = \"Recruitment deviations\",",
    "        xlim = c(min(Year), max(Year) + proyears), ylim = c(0, 1.1 * max(c(Perr, Perr_future))), zero_line = TRUE)",
    "matlines(max(Year) + 1:proyears, t(Perr_future), type = \"l\")",
...
rmd_RCM_fleet_output <- function(ff, f_name) {
  ans <- c(paste("### ", f_name[ff], "\n"),
           ...
           paste0("matplot(Year, FM, type = \"l\", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, ylim = c(0, 1.1 * max(FM, na.rm = TRUE)), xlab = \"Year\", ylab = \"Fishing Mortality of ", f_name[ff], "\", zero_line = TRUE)"),
           ...
           paste0("  matplot(Year, Cpred, col = scenario$col, type = \"l\", lty = scenario$lty, lwd = scenario$lwd, xlab = \"Year\", ylab = \"Predicted relative catch of ", f_name[ff], "\", zero_line = TRUE)"),
           "}",
           "if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
           "```\n",
           "",
           ...
           paste0("  lines(Year, MAobs, col = \"black\", type = \"o\")"),
           "}",
           "if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
           "```\n",
           "",
           ...
           "  lines(Year, MLobs, col = \"black\", type = \"o\")",
           "  if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
           "}",
           "```\n",
           "",
           ...
           "  lines(Year, MWobs, col = \"black\", type = \"o\")",
           "  if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
           "}",
           "```\n",
           "",
           ...
           paste0("if (any(RCMdata@CAA[, , ", ff, "] > 0, na.rm = TRUE)) {"),
           paste0("  plot_composition_RCM(Year, x@CAA[, , , ", ff, "], RCMdata@CAA[, , ", ff, "], N = round(RCMdata@CAA_ESS[, ", ff, "], 1), ages = Age, dat_color = scenario$col)"),
           "}",
           "```\n",
           paste0("```{r, fig.cap = \"Predicted age composition from ", f_name[ff], ".\"}"),
           paste0("if (any(RCMdata@CAA[, , ", ff, "] > 0, na.rm = TRUE)) {"),
           paste0("  plot_composition_RCM(Year, x@CAA[, , , ", ff, "], ages = Age, dat_color = scenario$col)"),
           "}",
           "```\n",
           "",
           ...
           paste0("  plot_composition_RCM(Year, fit = CAL_plot <- x@CAL[, , , ", ff, "], dat = RCMdata@CAL[, , ", ff, "], N = round(RCMdata@CAL_ESS[, ", ff, "], 1), CAL_bins = length_bin, dat_color = scenario$col)"),
           "}",
           "```\n")
...
rmd_RCM_index_output <- function(sur, s_name) {
  ans <- c(paste0("### ", s_name[sur], " \n"),
           ...
           paste0("  matplot(RCMdata@Misc$lbinmid, ivul_len, type = \"l\", xlab = \"Length\", ylim = c(0, 1), ylab = \"Selectivity of ", s_name[sur], "\", zero_line = TRUE)"),
           "  if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
           "}",
           "```\n",
           "",
           ...
           paste0("matplot(Year, Ipred, type = \"l\", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, ylim = c(0, 1.1 * max(c(Ipred, RCMdata@Index[, ", sur, "]), na.rm = TRUE)), xlab = \"Year\", ylab = \"", s_name[sur], "\", zero_line = TRUE)"),
           paste0("lines(Year, RCMdata@Index[, ", sur, "], col = \"black\", type = \"o\")"),
           "if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
           paste0("log_like <- sapply(report_list, function(x) sum(x$nll_index[, ", sur, ", 1]))"),
           ...
           paste0("  matplot(Year[ind], Ipred[ind, , drop = FALSE], type = \"l\", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, ylim = c(0, 1.1 * max(c(Ipred[ind, ], II[ind], err[ind, ]), na.rm = TRUE)), xlab = \"Year\", ylab = \"", s_name[sur], "\", zero_line = TRUE)"),
           ...
           paste0("  lines(Year, MLobs, col = \"black\", type = \"o\")"),
           "}",
           "if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
           "}",
           "```\n",
           ...
           paste0("  plot_composition_RCM(Year, pred_IAA, RCMdata@IAA[, , ", sur, "], N = round(RCMdata@IAA_ESS[, ", sur, "], 1), ages = Age, dat_color = scenario$col)"),
           "}",
           "```\n",
           "",
           ...
           paste0("  plot_composition_RCM(Year, pred_IAL, RCMdata@IAL[, , ", sur, "], N = round(RCMdata@IAL_ESS[, ", sur, "], 1), CAL_bins = length_bin, dat_color = scenario$col)"),
           "}",
           "```\n"
  )
  ans
...
rmd_RCM_SPR2 <- function() {
  c("```{r, fig.cap = \"Annual spawning potential ratio (SPR). Equilibrium SPR is calculated from the F-at-age in the corresponding year, while dynamic SPR is calculated from the cumulative survival of cohorts.\"}",
    "plot(Year, report$SPR_eq, type = \"o\", lty = 3, ylim = c(0, 1), xlab = \"Year\", ylab = \"Spawning potential ratio\", zero_line = TRUE)",
    "lines(Year, report$SPR_dyn, type = \"o\", pch = 16)",
...
rmd_RCM_Hist_compare <- function() {
  c("### RCM-OM comparison {.tabset}\n",
    ...
    "matplot(Year, t(Hist_F), type = \"l\", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, xlab = \"Year\", ylab = \"OM Apical F\", ylim = c(0, 1.1 * max(Hist_F)))",
    "abline(h = 0, col = \"grey\")",
    "if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
    "```\n",
    "",
    ...
    "matplot(Year, t(Hist_SSB), type = \"l\", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, xlab = \"Year\", ylab = \"OM SSB\", ylim = c(0, 1.1 * max(Hist_SSB)))",
    "abline(h = 0, col = \"grey\")",
    "if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
    "```\n",
    "",
    "```{r, fig.cap = \"Spawning biomass (SSB) relative to MSY from the operating model.\"}",
    "matplot(Year, t(Hist_SSB/Hist@Ref$ReferencePoints$SSBMSY), col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, type = \"l\", xlab = \"Year\", ylab = expression(OM~~SSB/SSB[MSY]), ylim = c(0, 1.1 * max(Hist_SSB/Hist@Ref$ReferencePoints$SSBMSY)))",
    ...
    "  matplot(Year[Yr_ind], t(Hist_SSB[, Yr_ind, drop = FALSE]/Hist@Ref$ReferencePoints$SSBMSY), type = \"l\", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, xlab = \"Year\", ylab = expression(OM~~SSB/SSB[MSY]), ylim = c(0, 1.1 * max(Hist_SSB[, Yr_ind, drop = FALSE]/Hist@Ref$ReferencePoints$SSBMSY)))",
    "  abline(h = c(0, MSY_ref), col = \"grey\")",
    "  if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
    "}",
    "```\n",
    ...
    "matplot(Year, t(Hist_SSB/Hist@Ref$ReferencePoints$SSB0), type = \"l\", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, xlab = \"Year\", ylab = expression(OM~~SSB/SSB[0]), ylim = c(0, 1.1 * max(Hist_SSB/Hist@Ref$ReferencePoints$SSB0)))",
    "abline(h = 0, col = \"grey\")",
    "if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
    "```\n",
    "",
    ...
    "matplot(Year, t(Hist_R), type = \"l\", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, xlab = \"Year\", ylab = \"OM Recruitment\", ylim = c(0, 1.1 * max(Hist_R)))",
    "abline(h = 0, col = \"grey\")",
    "if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
    "```\n",
    "",
    ...
    "matplot(Year, t(Hist_C), type = \"l\", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, xlab = \"Year\", ylab = \"OM Catch\", ylim = c(0, 1.1 * max(Hist_C)))",
    "abline(h = 0, col = \"grey\")",
    "if (!is.null(scenario$names)) legend(\"topleft\", scenario$names, col = scenario$col_legend, lty = scenario$lty, lwd = scenario$lwd)",
    "```\n",
    "",
    ...
    "matplot(Year, t(Hist_F), type = \"o\", pch = 16, col = \"red\", xlab = \"Year\", ylab = \"Apical F\", ylim = c(0, 1.1 * max(c(Hist_F, OM@cpars$Find))))",
    ...
    "matplot(Year, t(Hist_F - OM@cpars$Find), type = \"n\", xlab = \"Year\", ylab = \"Difference in apical F\")",
    "abline(h = 0, col = \"grey\")",
    "matlines(Year, t(Hist_F - OM@cpars$Find), col = \"black\")",
    "```\n",
    "",
    "```{r, fig.cap = \"SSB comparison between the OM and RCM.\"}",
    "matplot(Year, t(Hist_SSB), type = \"o\", col = \"red\", pch = 16, xlab = \"Year\", ylab = \"SSB\",",
    ...
    "matplot(Year, t((Hist_SSB - x@SSB[sims, 1:OM@nyears, drop = FALSE])/Hist@Ref$ReferencePoints$SSB0), type = \"n\", xlab = \"Year\", ylab = \"Difference in relative SSB\")",
    "abline(h = 0, col = \"grey\")",
    ...
    "matplot(Year, t(Hist_R), type = \"o\", col = \"red\", pch = 16, xlab = \"Year\", ylab = \"Recruitment\",",
    ...
    "matplot(Year, t(Hist_R/OM@cpars$R0 - x@NAA[, 1:OM@nyears, 1][sims, , drop = FALSE]/OM@cpars$R0), type = \"n\", xlab = \"Year\", ylab = \"Difference in relative recruitment\")",
    "abline(h = 0, col = \"grey\")",
    ...
    "matplot(Year, t(Hist_C), type = \"o\", col = \"red\", pch = 16, xlab = \"Year\", ylab = \"Total removals\",",
    ...
    "matplot(Year, Catch_difference, type = \"n\", xlab = \"Year\", ylab = \"Difference in relative removals\")",
    "abline(h = 0, col = \"grey\")",
