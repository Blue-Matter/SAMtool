---
title: "2. Description of the SRA model for conditioning MSEtool operating models"
...
vignette: >
  %\VignetteIndexEntry{2. Description of the SRA model for conditioning MSEtool operating models}
...
# Stock Reduction Analysis (SRA) for conditioning MSEtool operating models
...
In SAMtool, `SRA_scope()` will be the main function for scoping historical scenarios for an operating model `OM`. `SRA_scope` takes an operating model, data, in order to run the SRA and then returns a list with an updated OM and predicted outputs from the SRA. All model configurations for the SRA will also be specified through arguments passed through `SRA_scope`.
...
Currently, it is possible to create a more complex operating model than the SRA model itself. For example, discard mortality, movement, and spatial targetting are currently not modeled in the SRA. It is assumed that the catch in the SRA are all known sources of removal, i.e. landings plus discards. The SRA is a single area model, whereas MSEtool uses a multiple-area model. A simple operating model that best matches the SRA model may have the following configurations:
