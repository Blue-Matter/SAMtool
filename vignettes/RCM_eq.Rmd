---
title: "2a. Mathematical description of the RCM"
...
vignette: >
  %\VignetteIndexEntry{2a. Mathematical description of the RCM}
...
$$ h = 0.8 \times \dfrac{\exp(x^h)}{1 + \exp(x^h)} + 0.2. $$
...
including likelihood, parameter priors, and penalty functions, described below.
...
for mean weights, where $\eta_{y,f}$ is the standard deviation of the mean size. With constant cofficient of variation (CV), $\eta_{y,f} = \bar{w}^{\textrm{obs}}_{y,f} CV^{\eta}_f$.
...
A Beverton-Holt steepness prior uses a beta distribution on $y = (h - 0.2)/0.8$. The penalty to the objective function is:
$$\textrm{pr}^{h} = \log(y) (\alpha-1) + \log(1-y) * (\beta-1) - \dfrac{1}{y - y^2},$$
